    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      {!loading && results !== null && source && (
        <div className="flex items-center gap-2 mb-3">
          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
            source === 'google' ? 'bg-brand-blue/20 text-brand-blue' : 'bg-brand-border text-brand-muted'
          }`}>
      ...
      {!loading && results !== null && results.length > 0 && (
        <div className="bg-brand-surface rounded-xl border border-brand-border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-brand-bg border-b border-brand-border">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-brand-muted">Téléphone</th>
              ...
              {results.map(r => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    {r.address && <div className="text-xs text-brand-muted mt-0.5">{r.address}</div>}
                  </td>
          ...
          {selected.size > 0 && (
            <div className="px-4 py-3 border-t border-brand-border flex justify-end">
              <button onClick={handleImport}
                className="px-5 py-2 rounded-lg bg-emerald-700 text-white text-sm font-medium hover:bg-emerald-600 transition-colors">
                Importer les sélectionnés ({selected.size})
              </button>
            </div>
          )}
