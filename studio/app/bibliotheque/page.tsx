  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Link href="/studio?new=1"><Button tone="lime" icon="plus">Nouvelle création</Button></Link>}
      />
      ...
      {!list.length ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {list.map((c) => {
            ...
            return (
              <Link key={c.id} href={`/studio?id=${c.id}`} className="group flex flex-col rounded-[28px] bg-surface p-5 transition hover:bg-surface-2">
                ...
                <div className="mt-auto flex items-end justify-between pt-5 text-sm">
                  <span className="text-muted">{c.prompts.length} prompt(s) · {c.generations.length} génération(s)</span>
